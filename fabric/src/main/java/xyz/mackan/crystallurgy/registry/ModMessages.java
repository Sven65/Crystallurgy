package xyz.mackan.crystallurgy.registry;

import net.minecraft.util.Identifier;
import xyz.mackan.crystallurgy.Crystallurgy;

public class ModMessages {
    public static final Identifier ENERGY_SYNC = new Identifier(Crystallurgy.MOD_ID, "energy_sync");
    public static final Identifier FLUID_SYNC = new Identifier(Crystallurgy.MOD_ID, "fluid_sync");
    public static final Identifier SPAWN_PARTICLES = new Identifier(Crystallurgy.MOD_ID, "spawn_particles");
    public static final Identifier EXTRACT_FLUID_TO_CURSOR = new Identifier(Crystallurgy.MOD_ID, "extract_fluid_to_cursor");

    public static void register() {

    }
}
