    ...
    public static void receive(MinecraftServer server, ServerPlayerEntity player, ServerPlayNetworkHandler networkHandler, PacketByteBuf buf, PacketSender sender) {
        ...
        server.execute(() -> {
            ...
            if (Objects.equals(tankName, "input")) {
                FluidSynthesizerBlockEntity.extractInputFluid((FluidSynthesizerBlockEntity) be, FluidStack.convertDropletsToMb(FluidConstants.BUCKET));
            } else {
                FluidSynthesizerBlockEntity.extractOutputFluid((FluidSynthesizerBlockEntity)be, FluidStack.convertDropletsToMb(FluidConstants.BUCKET));
            }
