...
public class FluidSynthesizerCategory implements IRecipeCategory<FluidSynthesizerRecipe> {
    ...
    public static final Identifier TEXTURE = Constants.id("textures/gui/fluid_synthesizer.png");
    ...
    public FluidSynthesizerCategory(IGuiHelper helper) {
        ...
        this.icon = helper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(ForgeModBlocks.FLUID_SYNTHESIZER.get()));
    }
    ...
    private void assignFluidStackRenderers() {
        inputFluidRenderer = new FluidStackRenderer(FluidSynthesizerBlockEntity.INPUT_FLUID_CAPACITY, true, 15, 66);
        outputFluidRenderer = new FluidStackRenderer(FluidSynthesizerBlockEntity.OUTPUT_FLUID_CAPACITY, true, 15, 66);
    }

    @Override
    public void setRecipe(IRecipeLayoutBuilder builder, FluidSynthesizerRecipe fluidSynthesizerRecipe, IFocusGroup iFocusGroup) {
        Fluid outputFluid = fluidSynthesizerRecipe.getOutputFluid();
        int outputFluidAmount = fluidSynthesizerRecipe.getOutputFluidAmount();
        ...
        builder.addSlot(RecipeIngredientRole.OUTPUT).addFluidStack(outputFluid, outputFluidAmount).setPosition(Integer.MAX_VALUE, Integer.MAX_VALUE);
        builder.addSlot(RecipeIngredientRole.OUTPUT, 7, 54).addItemStack(outputFluid.getBucketItem().getDefaultStack());
    ...
    public void draw(FluidSynthesizerRecipe recipe, IRecipeSlotsView recipeSlotsView, DrawContext guiGraphics, double mouseX, double mouseY) {
        ...
        FluidStack inputFluidStack = new FluidStack(recipe.getInputFluid(), recipe.getInputFluidAmount());
        FluidStack outputFluidStack = new FluidStack(recipe.getOutputFluid(), recipe.getOutputFluidAmount());

        inputFluidRenderer.drawFluid(guiGraphics, inputFluidStack, 51, 9, 9, 66,
                FluidSynthesizerBlockEntity.INPUT_FLUID_CAPACITY);

        outputFluidRenderer.drawFluid(guiGraphics, outputFluidStack,  129, 9, 9, 66,
                FluidSynthesizerBlockEntity.OUTPUT_FLUID_CAPACITY);



        if (isMouseAboveArea((int) mouseX, (int) mouseY, 51, 9, inputFluidRenderer)) {
            List<Text> tooltip = inputFluidRenderer.getTooltip(inputFluidStack, TooltipContext.Default.BASIC);
        ...
        if (isMouseAboveArea((int) mouseX, (int) mouseY, 129, 9, outputFluidRenderer)) {
            List<Text> tooltip = outputFluidRenderer.getTooltip(outputFluidStack, TooltipContext.Default.BASIC);
            guiGraphics.drawTooltip(client.textRenderer, tooltip, (int) mouseX, (int) mouseY);
        }
    }
